  ...
  return (
    <section className={`${poppins.className} space-y-4`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {(loading ? Array.from({ length: 4 }) : playlists.slice(0, 4)).map((fp: any, i: number) => (
          <Link key={i} href="/playlist" className="rounded-2xl bg-white/5 backdrop-blur border border-white/10 overflow-hidden hover:bg-white/10 transition">
            <div className="relative w-full aspect-[4/3]">
              {loading ? (
                ...
                <CldImage src={fp.cover} alt={fp.title} fill className="object-cover" />
              )}
            </div>
            <div className="p-4">
              <div className="text-white text-sm font-semibold">{loading ? "Loading..." : fp.title}</div>
            </div>
          </Link>
        ))}
      </div>
